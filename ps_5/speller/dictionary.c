...
bool load(const char *dictionary)
{
    ...
    // First, to add new data to hash table, allocate memory for nodes then add data to the node. this format copies words into the
    // character array strcpy(n->word, "hello"); to set next pointer n->next = NULL;
...
bool unload(void)
{
    ...
    // make a cursor which points to the head node. this cursor will work through the linked lists making each = NULL, freeing the
    // memory.
    node *tmp = NULL;
    node *cursor = NULL;
